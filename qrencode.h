 ...
 * 
 * 有两种方式 encode data: <b>encoding a string</b> or
 ...
 * by calling QRinput_Struct_new()。这个对象能容纳 multiple ::QRinput,
...
typedef enum {
    ...
    QR_MODE_NUM = 0,   ///< 数字模式
    QR_MODE_AN,             ///< Alphabet-numeric mode混合字符模式【数字，26个大写字母、9个特殊符号】
    QR_MODE_8,                ///< 8-bit 数据模式
    QR_MODE_KANJI,        ///< Kanji (shift-jis) 日本汉字模式
 ...
 */
typedef enum {              //容错级别
    QR_ECLEVEL_L = 0, ///< Lowest级别，约可纠错7%的数据码字
    QR_ECLEVEL_M,     ///< Middle级别，约可纠错15%的数据码字
    QR_ECLEVEL_Q,      ///< Quality级别，约可纠错25%的数据码字
    QR_ECLEVEL_H       ///< Highest级别，约可纠错30%的数据码字
} QRecLevel;
...
struct _QRcode_List {	QRcode *code;	QRcode_List *next;  };
...
/**
 * Create a symbol from the string.本库自动解析输入字符串，并 encodes in a QR Code symbol.
 * @warning 警告：本函数是线程不安全的！！！
 * @param string 输入字符串。必须以 NULL 结尾.
 * @param version 符号的版本大小。如果为0，库会根据给定的输入数据，选择选择最小版本。
 * @param level 纠错水平。
 * @param hint 告诉这个库，非混合字符应该怎样编码。如果给出 QR_MODE_KANJI，kanji 字符
 * 就会 按照 Shif-JIS 字符编码。如果选择 If QR_MODE_8 ，所有的非混合字符就会维持原状。
 * 如果你想要 嵌入 UTF-8 字符串，请选择此项。
 * @param casesensitive   大小写敏感case-sensitive(1) 或者大小写不敏感(0)。
 * @return 一个QRcode 类的实例。结果 QRcode 的版本可能大于the designated version. 如果发生错误，返回NULL，而且错误代码可以用于显示错误原因。参见Exceptions 以了解更多细节。
 * @throw EINVAL invalid input object——无效的输入对象.
 * @throw ENOMEM unable to allocate memory for input objects——不能为输入对象分配内存.
 */
